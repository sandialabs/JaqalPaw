...
async def FirmwareEmulator(data_list, num_channels=8, num_dtypes=8, master_data_record=None):
    ...
    spline_fifos, gseq_fifos, dma_queue = construct_fifos(num_spline_fifos=num_dtypes,
...
def printMetadataOutput(input_bytes):
    ...
    for chnm in range(8):
        for n in range(1): # wait trig masks apply to all parameters
            ...
            for idx, t in enumerate(master_data_record[chnm][n]['time']):
                if master_data_record[chnm][n]['waittrig'][idx]:
                    tlist.append(t)
            print(f"ch {chnm} received {len(tlist)} wait trigger events at times: {tlist} diff: {np.diff(tlist)}"
    ...
    for chnm in range(8):
        for tone, n in enumerate([0,4]):#range(1):
            ...
            if elist:
                print(f"ch {chnm} tone {tone} received {len(elist)} enable mask events at times: {elist}")
                slstr = "state: "
                tstr =  " time: "
                deltastr = "\u0394time: "
                lastt = 0
...
def plotOctetEmulatorOutput(ret, compare_lut_to_bypass=False, num_plots=8):
    ...
    ls_list = 2*[{'linestyle':'-', 'where':'post'}, {'linestyle':'-.', 'where':'post'}, {'linestyle':'--', 'where':'post'}, {'linestyle':':', 'where':'post'}]
    for chnm in range(num_plots):
        for i in range(4):
            for j in range(2):
                jp, ip = plot_loc_dict[i+j*4]
                axl[ip][jp].set_ylabel(mod_type_dict[i+j*4]['name'])
                axl[ip][jp].step(time_scale*np.array(mdr[chnm][i+j*4]['time']),
                                 mdr[chnm][i+j*4]['data'],
                                 #where='post',
                                 **ls_list[chnm])
                #if compare_lut_to_bypass:
