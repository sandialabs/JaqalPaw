...
from QSCOUTBuiltins import QSCOUTBuiltins, GLOBAL_BEAM, both_tones, tone0
...
class HelperFunctions:
    ...
    def global_amp_cal(angle):
        ...
        return np.arcsin((omega_pi2/a)*np.sqrt(abs(angle)/(np.pi/2)))/(b*100)
        

class MSExemplar(QSCOUTBuiltins, HelperFunctions):
    
    MS_phi: float = 0.0
    def gate_MS(self, channel1, channel2, axis_rad=0, angle = np.pi/2, is_gaussian=True):
        
        #First discretize the frequencies to ensure frequency matching and prevent slow phase drift
        MS_freq0_total_corr = discretize_frequency(self.ia_center_frequency) + discretize_frequency(self.lower_motional_mode_frequencies[1]) + discretize_frequency(self.MS_delta) #Blue (lowest frequency mode)
        MS_freq1_total_corr = discretize_frequency(self.ia_center_frequency) - discretize_frequency(self.lower_motional_mode_frequencies[1]) - discretize_frequency(self.MS_delta) #Red (lowest frequency mode)
                
        # Use global beam as the lower leg of the Raman transition.
        global_beam_frequency = discretize_frequency(self.ia_center_frequency) - discretize_frequency(self.adjusted_carrier_splitting)
        ...
        if is_gaussian:
            ...
            framerot_input = tuple(self.erf(13, self.MS_framerot*((global_amp_scale)**3), freqwidth=300e3, total_duration=4e-6))
            ...
            framerot_app = 0b11 #Applies entire Stark shift correction at end of the pulse
