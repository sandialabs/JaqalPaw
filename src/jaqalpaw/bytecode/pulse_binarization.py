...
def apply_metadata(
    ...
    if ind == 0:
        ...
        if modtype & 0b11000000:  # frame_rot (z rotations)
            ...
            apply_at_eof = (
                1 << APPLY_EOF_LSB_LOC if (apply_at_eof_mask & (modtype >> 6)) else 0
            )
...
def pulse(
    ...
    mpdict = {
        ...
        "phase0": {
            "modtype": PHSMOD0,
            "data": phase0,
            "convertFunc": convert_phase_full,
            "enabled": True,
        },
        "phase1": {
            "modtype": PHSMOD1,
    ...
    for modt in modlist:
        ...
        if n_points > 1:
            ...
            lbytes, _ = generate_single_pulse_bytes(
                mpdict[modt]["convertFunc"](delist(mpdict[modt]["data"])),
                dur,
                modtype=mpdict[modt]["modtype"],
                waittrig=waittrig,
                bypass=bypass,
