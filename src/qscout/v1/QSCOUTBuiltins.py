...
class UtilityPulses:
    ...
    def gate_CounterProp(self, channel, angle=np.pi, phase=0):
        global_freq = discretize_frequency(self.ia_center_frequency) - discretize_frequency(self.adjusted_carrier_splitting)
        duration = (angle/np.pi)*self.counter_resonant_pi_time
        return [PulseData(GLOBAL_BEAM, duration=duration,
                        freq0=global_freq,
                        amp0=self.amp0_counterprop,
                        phase0=phase,
                        sync_mask=both_tones),
                PulseData(channel, duration=duration,
